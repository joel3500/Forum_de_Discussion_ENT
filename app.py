...
from models import initialize, User, Topic, Comment, now_utc_naive, TopicMedia
...
MAX_FILE_SIZE  = 5 * 1024 * 1024          # 5 Mo par fichier
ALLOWED_IMG    = {'jpg', 'jpeg', 'png', 'gif', 'webp'}
ALLOWED_VID    = {'mp4', 'webm', 'mov', 'avi'}

def allowed_media(filename):
    ext = filename.rsplit('.', 1)[-1].lower() if '.' in filename else ''
    if ext in ALLOWED_IMG: return ext, 'image'
    if ext in ALLOWED_VID: return ext, 'video'
    return None, None
...
# ----- Nettoyage fichiers médias -----
def _delete_topic_media_files(topic):
    upload_path = os.path.join(app.static_folder, 'uploads')
    for m in TopicMedia.select().where(TopicMedia.topic == topic):
        fpath = os.path.join(upload_path, m.filename)
        try:
...
def get_saguenay_news_daily():
    ...
    today_local = now_utc_naive().replace(tzinfo=timezone.utc).astimezone(LOCAL_TZ).date()
...
@app.route('/')
def index():
    # Sujets avec médias préchargés (évite N+1 queries)
    sujets = list(prefetch(
        Topic.select().order_by(Topic.created_at.desc()),
        TopicMedia.select()
    ))
    return render_template('forum_de_discussion.html', sujets=sujets, user=current_user())
...
def view_topic(topic_id):
    ...
    topic_media = list(TopicMedia.select().where(TopicMedia.topic == sujet))
...
def nouveau_sujet():
    ...
    t = Topic.create(user=u, nom=nom, titre=titre, contenu=contenu)

    upload_path = os.path.join(app.static_folder, 'uploads')
    for f in request.files.getlist('media'):
        if not f or not f.filename:
            continue
        ext, mtype = allowed_media(secure_filename(f.filename))
        if not ext:
            flash(f"'{f.filename}' ignoré : type non supporté (jpg/png/gif/webp/mp4/webm/mov).")
            continue
        data = f.read()
        if len(data) > MAX_FILE_SIZE:
            flash(f"'{f.filename}' ignoré : dépasse 5 Mo.")
            continue
        unique_name = f"{uuid.uuid4().hex}.{ext}"
        with open(os.path.join(upload_path, unique_name), 'wb') as out:
            out.write(data)
        TopicMedia.create(topic=t, filename=unique_name, media_type=mtype)
