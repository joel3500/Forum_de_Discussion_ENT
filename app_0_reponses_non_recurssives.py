...
from models_0_reponses_non_recurssives import initialize, User, Topic, Comment, Reply, now_utc_naive
...
def view_topic(topic_id):
    ...
    # réponses triées (du plus récent au plus ancien) pour ces commentaires
    replies_q = (Reply
                 .select()
                 .where(Reply.comment.in_(comments_q))
                 .order_by(Reply.created_at.desc()))

    # attache les replies triées à chaque comment (c.replies devient une liste déjà triée)
    comments = list(prefetch(comments_q, replies_q))

    return render_template('sujet.html', sujet=sujet, comments=comments, user=current_user())
...
def commenter(topic_id):
    ...
    if not (nom and contenu):
        return redirect(url_for('view_topic', topic_id=topic_id))
    Comment.create(topic=topic_id, user=u, nom=nom, contenu=contenu, country=country, city=city)
    return redirect(url_for('view_topic', topic_id=topic_id))

@app.route('/repondre/<int:comment_id>', methods=['POST'])
def repondre(comment_id):
    u = current_user()
    if not u:
    ...
    Reply.create(comment=comment_id, user=u, nom=nom, contenu=contenu, country=country, city=city)
    return redirect(request.referrer or url_for('index'))
...
def edit_comment(comment_id):
    ...
    new_text = request.form.get('contenu')
    if new_text:
        c.contenu = new_text
        c.save()
...
def admin_home():
    ...
    replies = Reply.select().order_by(Reply.created_at.desc())
    users = User.select().order_by(User.id.desc())
    return render_template('admin.html', sujets=sujets, comments=comments, replies=replies, users=users, user=current_user())
...
def admin_delete_reply(reply_id):
    r = admin_required()
    if r: return r
    rp = Reply.get_or_none(Reply.id == reply_id)
    if rp:
        rp.delete_instance()
        flash("Réponse supprimée (admin).")
    return redirect(url_for('admin_home'))
