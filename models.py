...
# ---------- Médias attachés à un sujet ----------
class TopicMedia(BaseModel):
    topic      = ForeignKeyField(Topic, backref='media', on_delete='CASCADE')
    filename   = CharField()                 # nom unique sur disque (uuid.ext)
...
def initialize():
    with db:
        db.create_tables([User, Topic, Comment, NewsCache, TopicMedia])
